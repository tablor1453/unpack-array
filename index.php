...
foreach($dataRaw as $d){
    for ($i=0; $i < count($arrayTahun); $i++) { 
        ...
        if($d['tahun_neraca'] == '2015'){
            ...
            $tmpTahun['total_bijih_sd'] += $d['jumlah_bijih_SD'];
            $tmpTahun['total_bijih_cd'] += $d['jumlah_bijih_CAD'];
            $tmpTahun['total_logam_sd'] += $d['jumlah_logam_SD'];
            $tmpTahun['total_logam_cd'] += $d['jumlah_logam_CAD'];
            $i++;
        }
    }
